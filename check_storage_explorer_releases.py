#!/usr/bin/env python3
"""
Check for new Azure Storage Explorer releases and alert Slack via webhook.

Data source: GitHub Releases API for the Azure Storage Explorer repo (microsoft/AzureStorageExplorer).
  https://api.github.com/repos/microsoft/AzureStorageExplorer/releases/latest
...
Environment variables:
  ...
  REPO                (optional) owner/name to watch. Default "microsoft/AzureStorageExplorer"
                      (the standalone Azure Storage Explorer desktop app). Change this
                      if you ever want to point this engine at a different repo.
  INCLUDE_PRERELEASE  (optional) "true" to also alert on prereleases/betas.
                      Default "false" (stable releases only).
  STATE_FILE          (optional) Default "storage-explorer-release-state.json".
"""
...
REPO = os.environ.get("REPO", "microsoft/AzureStorageExplorer").strip()
STATE_FILE = os.environ.get("STATE_FILE", "storage-explorer-release-state.json")
...
USER_AGENT = "moneybox-storage-explorer-release-checker"
...
def build_blocks(rel):
    ...
    icon = ":test_tube:" if is_pre else ":cloud:"
    title = f"{icon} *Azure Storage Explorer — {tag}*"
    if is_pre:
        title += "  `prerelease`"
    if name and name != tag:
        title += f"\n{name}"

    header = ":cloud: *New Azure Storage Explorer release*"
...
def main():
    ...
    fallback = f"New Azure Storage Explorer release: {tag}"
    try:
